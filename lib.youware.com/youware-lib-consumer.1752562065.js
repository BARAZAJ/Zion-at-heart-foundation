! function(e) {
    ...
    async function l(l, d) {
        let g, c, u = null;
        if (l instanceof Request ? (c = l, g = c.url, u = l.body, d && ("body" in d && (u = d.body), c = new Request(c, d))) : (g = l instanceof URL ? l.toString() : l, u = null == d ? void 0 : d.body, c = new Request(g, d)), ! function(e) {
                return a.urlPatterns.some((t => e.includes(t)))
            }(g)) return i(l, d);
        const h = `${Date.now()}-${Math.random().toString(36).substring(2,9)}`;
        try {
            const i = new AbortController;
            s.set(h, i), c.signal && c.signal.addEventListener("abort", (() => {
                i.abort(), n.sendEvent("fetch-proxy:stream-control", {
                    requestId: h,
                    action: "abort"
                })
            }));
            const a = await async function(r) {
                    if (null == r) return {
                        type: "null",
                        data: null
                    };
                    ...
                    if (r instanceof URLSearchParams) return {
                    ...
                    if (r instanceof ArrayBuffer) return {
                        type: "arraybuffer",
                        data: t(r)
                    };
                    ...
                    throw new Error(`Unsupported body type: ${Object.prototype.toString.call(r)}`)
                }(u),
                l = {
                    requestId: h,
                    url: g,
            ...
            if (!n) throw new Error("Fetch proxy not initialized. Call initializeFetchInterceptor with messageManager first.");
            const d = await n.invoke("fetch-proxy:fetch", l),
                ...
                p = function(e, t = {}, n, i) {
                    ...
                    return new ReadableStream({
                        async start(t) {
                            l = t;
                            const r = n.onEvent("fetch-proxy:stream-control", (t => {
                            ...
                            i && i.addEventListener("abort", a), d = () => {
                                r(), i && i.removeEventListener("abort", a)
                            }
                        },
                        async pull(t) {
                            if (!o) try {
                                const r = await n.invoke("fetch-proxy:stream-read", {
                                    requestId: e,
                                    chunkIndex: s
                                });
                                if (r.done) o || (t.close(), o = !0, null == d || d());
                                else {
                                    const e = new Uint8Array(r.chunk);
                                    t.enqueue(e), s++
                                }
                            } catch (r) {
                                console.error("[Stream Proxy] Error in pull:", r), o || (t.error(r), o = !0, null == d || d())
                            }
                        },
                        cancel(t) {
                            o = !0, n.sendEvent("fetch-proxy:stream-control", {
                                requestId: e,
                                action: "abort",
    ...
    function d(e) {
        ...
        t = e, a = { ...a,
            ...t
        }, t.messageManager && (n = t.messageManager), t.urlPatterns && (a.urlPatterns = t.urlPatterns), window.fetch = l, "undefined" != typeof globalThis && (globalThis.fetch = l)
    }
    const g = new class {
        constructor(e) {
            var t;
        ...
        sendEventWithTransferable(e, t, r, n = 3, i = 1e3) {
            ...
            null == (a = this.logger) || a.info("Sending event with transferable", {
                event: e,
                messageId: s,
                dataType: typeof t,
        ...
        invoke(e, t) {
            return new Promise(((r, n) => {
                ...
                this.pending.set(s, {
                    timer: l,
                    attempt: 0
                }), null == (a = this.targetWindow) || a.postMessage(o, this.targetOrigin)
            }))
        }
        invokeWithTransferable(e, t, r) {
            return new Promise(((n, i) => {
                ...
                null == (a = this.logger) || a.info("Invoking remote method with transferable", {
                    event: e,
                    messageId: o,
                    dataType: typeof t,
                    transferableCount: r.length
                ...
                this.pending.set(o, {
                    timer: d,
                    attempt: 0
                }), null == (s = this.targetWindow) || s.postMessage(l, this.targetOrigin, r)
            }))
        }
        sendMessageWithRetry(e, t, r, n) {
            const i = (a = 0) => {
                var s, o;
                if (a > 0 && (null == (s = this.logger) || s.warn("Message retry attempt", {
                        event: e.event,
                        messageId: e.id,
                        attempt: a + 1,
                        maxAttempts: t + 1
                    ...
                    const n = setTimeout((() => {
                        var r;
                        if (a >= t) return this.pending.delete(e.id), void(null == (r = this.logger) || r.error("Message failed after all retries", {
                            event: e.event,
                            messageId: e.id,
                            totalAttempts: a + 1
                        }));
        ...
        async handleInvoke(e, t) {
            ...
            try {
                ...
                }), null == (a = this.targetWindow) || a.postMessage(d, t, g)) : null == (s = this.targetWindow) || s.postMessage(d, t)
            } catch (d) {
    ...
    ! function() {
        try {
            d({
                urlPatterns: ["api.youware.com/public/v1/ai/"],
                messageManager: g
            })
        } catch (e) {
    ...
    const c = {
        innerRequest: function(e, t) {
            ...
            return g.invoke("innerRequest", {
                apiName: e,
                params: t
            })
        },
        getLoginStatus: async function() {
            return g.invoke("getLoginStatus", void 0)
        },
        requestLogin: async function(e) {
            return g.invoke("requestLogin", e)
        }
    };
    ...
    function h(e) {
        return null === e || "object" != typeof e ? e : (Object.keys(e).forEach((t => {
            null !== e[t] && "object" == typeof e[t] && h(e[t])
        })), Object.freeze(e))
    }
